...
const Navbar = () => {
  ...
  return (
    <header 
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ease-in-out ${bgColorClass} ${isOpen ? 'h-screen' : 'h-auto'}`}
    >
      <div className="flex items-center justify-between px-6 py-6">
        ...
        >
          {isOpen ? <Plus size={32} strokeWidth={1} /> : <Menu size={32} strokeWidth={1} />}
        </button>
      </div>
